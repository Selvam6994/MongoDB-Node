...
//operator key - always starts with $
...
// Note:donot mix inclusion and exclusion in same command
//Note 2 : but the above constrain not applicable for _id
db.movies.find({rating:{$lt:8.5}},{_id:1,name:0,rating:0})

db.movies.find({rating:{$lte:8.5}},{_id:1,name:1,rating:1}).pretty()

//in operator in mongodb
db.movies.find({rating:{$in:[8,8.8,8.6]}},{_id:0,name:1,rating:1})

//notIn operator in mongodb
db.movies.find({rating:{$nin:[8,8.8,8.6]}},{_id:0,name:1,rating:1})

//movies rating with exactly 8
db.movies.find({rating:{$in:[8]}},{_id:0,name:1,rating:1})

//movies rating exept 8
db.movies.find({rating:{$ne:8}},{_id:0,name:1,rating:1})

//movie rating range > 8.5 & 9 <
...
db.order_collection.aggregate([
  {$match:{status:"urgent"}},
  {$group:{_id:"$production",totalUrgentQuantity:{$sum:"$quantity"}}}
])


//delete
db.movies.deleteOne({name:"The Avengers"})


//update

db.movies.updateMany({},{$set:{language:"English"}})
db.movies.find({},{_id:0,name:1,rating:1,language:1}).sort({rating:1})
...
db.movies.deleteOne({ _id: ObjectId("63eb187c6cc4e6a04a8c1a3c")})

db.movies.find({_id: ObjectId("63eb187c6cc4e6a04a8c1a3c")})